...
from server.create_shiprocket_for_sales_orders import fetch_shiprocket_order_detail

def shipment_order_component():
    with st.container():
        st.header("Shipment Order Details")
        shipment_order_df=fetch_shiprocket_order_detail()
        shipment_order_df['created_at'] = pd.to_datetime(shipment_order_df['created_at'], errors='coerce')
        apply_advanced_filtering(shipment_order_df)
...
def apply_advanced_filtering(df):
    ...
    if st.button(key="shipment-order-component",label="Download Filtered Data"):
        csv = filtered_df.to_csv(index=False)
        st.download_button(
            key="shipment-order-component",
            label="Download CSV",
            data=csv,
